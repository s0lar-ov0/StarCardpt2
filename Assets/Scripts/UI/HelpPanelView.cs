...
namespace StarCard.UI
{
    ...
    public class HelpPanelView : MonoBehaviour
    {
        ...
        private static string RulesText() =>
            ...
            "  方位祝福的效果统一是<b>屏蔽一类随机事件</b>：青龙挡左右平移、白虎挡上下平移、朱雀挡九宫格旋转、玄武挡对称变换。\n" +
            "· <b>行动</b>：放置手牌 / 移动棋盘上的牌 / 交换两张牌，各算一次行动。行动次数用尽即自动结束回合。\n" +
            "· <b>漂移</b>：回合结束时，棋盘上所有非连结的牌随机移动到其他合法空位 —— 也可能因此凑成连结甚至归位。\n" +
            "  漂移结束后会停顿两秒让你看清结果，再进入下一回合（按空格可跳过）。\n" +
            "· <b>手牌不跨回合</b>：回合结束时手上没放下的星宿牌全部退回卡池，下回合要靠流星重新获取。\n" +
            ...
            "  小型粉色流星：未满 3 个祝福时，每颗都能点，抽到的进候选（每回合最多 3 个）；结算界面从候选里<b>选一个</b>收下，没选的下回合还能再抽到。\n" +
            "  已有 3 个祝福时点中粉色流星，可在结算界面把其中一个升 1 级（点多颗也只升一次），满级的不能再升。\n" +
